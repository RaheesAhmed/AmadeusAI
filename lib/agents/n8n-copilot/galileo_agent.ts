...
import { GALILEO_SYSTEM_PROMPT } from "./prompts/galileo_system_prompt";
...
/**
 * Create Galileo Agent with specified configuration
 */
export function createGalileoAgent(config: N8nCopilotConfig = {}): any {
  ...
  const agent = createReactAgent({
    ...
    stateModifier: GALILEO_SYSTEM_PROMPT,
    llm: model,
 ...
 */
export function validateGalileoEnvironment(): {
  isValid: boolean;
  errors: string[];
  warnings: string[];
} {
...
export async function* streamWorkflowGeneration(
  ...
  const agent = createGalileoAgent(config);
...
export default createGalileoAgent;